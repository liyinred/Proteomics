...
- **转录（Transcription）**：在细胞中，DNA序列被转录成mRNA的过程。这是蛋白质合成的第一步，涉及到从DNA信息转换成RNA的过程。遗传信息从DNA转录到信使RNA（mRNA）上。转录是指以DNA为模板，合成RNA分子。
义链（Sense Strand）：与mRNA序列相同，但不作为转录模板，仅提供编码信息的参考。
无义链（Antisense Strand）：作为转录模板，通过互补配对指导mRNA的合成。
...
- DNA和RNA的核苷酸由一个磷酸基团、一个五碳糖（脱氧核糖或核糖）和一个碱基组成。磷酸基团连接在五碳糖的5'碳原子上，而碱基连接在1'碳原子上。3'碳原子上有一个羟基（-OH）。
核苷酸添加反应：
- 在DNA或RNA合成过程中，新的核苷酸通过磷酸二酯键连接到正在延伸的核酸链上。这一反应发生在链的3'末端，即每次新的核苷酸都添加到3'羟基上。
具体来说，新的核苷酸的5'磷酸基团与前一个核苷酸的3'羟基反应，形成一个新的磷酸二酯键，释放一个焦磷酸（PPi）分子。这个反应由DNA聚合酶（或RNA聚合酶）催化。
...
**IgG中一共有16个二硫键,链间 二硫键最容易断,其次是铰链区中间上面那条,再是下面那条,最难的是最下面的那条。**
...
### 1. De Novo（从头测序）
从头测序是一种不依赖于已知蛋白质数据库，通过质谱数据直接推断蛋白质或肽段序列的方法。它对于未知的或修饰的蛋白质分析尤为重要。这一步通过分析质谱图中的碎片离子，逐步推断氨基酸序列。

### 2. Denovo Consolidation（从头测序整合）
从头测序整合是将多个从头测序结果进行汇总和优化，以获得更精确的序列信息。通过结合多次分析的结果，可以减少错误并提高测序的覆盖度和准确性。

### 3. Peptide Selection（肽段选择）
肽段选择涉及从质谱数据中选择符合分析标准的肽段。这些标准可能包括信号强度、碎片覆盖度、肽段长度等。选择的肽段将用于后续的序列组装和比对。

### 4. Sequence Assembly（序列组装）
序列组装是将选定的肽段序列拼接成完整的蛋白质序列。这一步骤通常利用计算算法来确定最佳的肽段排列，以重构蛋白质的初级结构。

### 5. Protein Alignment（蛋白质比对）
蛋白质比对是将组装好的蛋白质序列与已知的参考蛋白质数据库进行比较。这有助于确定蛋白质的身份、同源性以及潜在功能。比对过程中会考虑氨基酸替换、缺失和插入等情况。

### 6. Sequence Refinement（序列精炼）
序列精炼是在初步比对的基础上，对蛋白质序列进行进一步的修正和优化。包括处理序列中的模糊区域、校正错误的氨基酸序列等，以提高数据的准确性。

### 7. I/L Differentiation（I/L 区分）
在质谱分析中，异亮氨酸（Isoleucine, I）和亮氨酸（Leucine, L）具有相同的质量，难以区分。这一步骤通过特定的方法或软件工具来区分这两种氨基酸，从而提高序列解析的准确性。

### 8. Peptide Mapping（肽段映射）
肽段映射是将质谱检测到的肽段序列与参考蛋白质序列进行位置对照。这有助于确定每个肽段在蛋白质中的具体位置及其覆盖的区域。

### 9. Region Annotation（区域注释）
区域注释是对蛋白质序列中的特定功能区域进行标记和注释，如酶活性位点、结合位点、信号肽等。这一步可以帮助解释蛋白质的功能及其在生物过程中可能的角色。

### 10. I/L Statistics（I/L 统计）
I/L 统计分析是对I/L区分的结果进行统计评估，提供关于区分准确性的报告。这对于验证分析结果的可靠性和质量非常重要。

### 11. Result Generation（结果生成）
结果生成是将所有分析数据和结果整合成最终报告，包括定性和定量分析、统计数据、序列信息等。这些报告通常用于进一步的生物学研究或应用。

### 12. PEAKS Glycan（糖基化分析）
PEAKS Glycan是一个专门用于分析蛋白质糖基化修饰的模块。它可以识别和量化蛋白质上的糖基化位点和糖基类型，这对于研究蛋白质功能和疾病相关的糖基化变异非常关键。

...
- 去污（如用甲醇和乙酸）以去除背景染色。
- 肽段纯化（Purification）：消化后的肽段需要纯化，去除杂质和缓冲液成分。

7. 样本浓缩与干燥：纯化后的肽段需要浓缩和干燥，以便于后续的质谱分析。

8. 质谱分析准备：干燥后的肽段重新溶解在适当的溶剂中（如0.1%甲酸溶液），准备进行液相色谱-质谱联用（LC-MS/MS）分析。

9. 质谱数据分析：通过LC-MS/MS获取质谱数据后，利用专业的软件（如Mascot、MaxQuant）进行数据分析和肽段鉴定
